...
import json, sys, os, statistics, calendar
...
def build_monthly_maps(conn, year_filter=None):
    """Ritorna inc_map e spe_map {YYYY-MM: valore} per un anno o per tutti."""
    where = f"AND strftime('%Y',date)='{year_filter}'" if year_filter else ""
    inc_rows = q(conn, f"""
        SELECT strftime('%Y',date) y, strftime('%m',date) m, SUM(euro)
        FROM incomes WHERE user_id=1 {where} GROUP BY y,m ORDER BY y,m
    """)
    spe_rows = q(conn, f"""
        SELECT strftime('%Y',e.date) y, strftime('%m',e.date) m, SUM(e.euro)
        FROM expenses e WHERE e.user_id=1 {where} GROUP BY y,m ORDER BY y,m
    """)
...
def index():
    ...
    # Anno selezionato per i tab filtrabili per anno singolo
    ...
    COLORS = ['#7c83f5','#34d399','#fb923c','#f472b6','#38bdf8']
    ...
    for i, yr in enumerate(sorted(anni_sel)):
        ...
        m_tot   = {}
        for r in yr_rows: m_tot[int(r[1])] = m_tot.get(int(r[1]),0) + r[3]
        if m_tot:
            mx=max(m_tot,key=m_tot.get); mn=min(m_tot,key=m_tot.get)
            max_info=f"{MESI_IT[mx-1]}: €{m_tot[mx]:,.0f}"; min_info=f"{MESI_IT[mn-1]}: €{m_tot[mn]:,.0f}"
        else:
            max_info=min_info='N/D'
        delta_str='–'
        if i > 0:
        ...
        stats.append({'anno':yr,'totale':total,'media':avg_m,'max':max_info,'min':min_info,'delta':delta_str})

    cum_series, monthly_series = [], []
    for i, yr in enumerate(sorted(anni_sel)):
        yr_rows=sorted([r for r in frows if int(r[0])==yr],key=lambda r:r[2])
        doy_map={}
        for r in yr_rows: doy_map[r[2]]=doy_map.get(r[2],0)+r[3]
        days=sorted(doy_map.keys()); cumul=0; xs=[]; ys=[]
        for d in days: cumul+=doy_map[d]; xs.append(d); ys.append(round(cumul,2))
        cum_series.append({'year':str(yr),'x':xs,'y':ys,'color':COLORS[i%len(COLORS)]})
        m_tot={m:0 for m in range(1,13)}
        for r in yr_rows: m_tot[int(r[1])]+=r[3]
        monthly_series.append({'year':str(yr),'values':[round(m_tot[m],2) for m in range(1,13)],'color':COLORS[i%len(COLORS)]})

    # ── Tab 2: Saving Rate (anno selezionato o tutti) ───────────────
    sr_year = request.args.get('sr_year', '')  # '' = tutti
    ...
    for ym in all_months_sr:
        inc=inc_map_sr.get(ym,0); spe=spe_map_sr.get(ym,0); sav=inc-spe
        rate=(sav/inc*100) if inc>0 else 0
        sr_labels.append(ym); sr_rates.append(round(rate,1))
        sr_inc.append(round(inc,2)); sr_spe.append(round(spe,2)); sr_sav.append(round(sav,2))
    recent_rates=[r for r in sr_rates if r!=0][-12:]
    avg_sr=round(sum(recent_rates)/len(recent_rates),1) if recent_rates else 0
    saving_rate_data=json.dumps({'labels':sr_labels,'rates':sr_rates,'income':sr_inc,'spese':sr_spe,'savings':sr_sav})

    # ── Tab 3: Proiezione (anno selezionato) ────────────────────────
    ...
    # ── Tab 4: Nec vs Extra (anno selezionato) ──────────────────────
    ne_year = request.args.get('ne_year', '')
    ne_where = f"AND strftime('%Y',e.date)='{ne_year}'" if ne_year else ""
    ...
    for r in nec_ext_rows:
        k=f"{r[0]}-{r[1]}"
        if r[2]=='essential': nec_map2[k]=r[3]
        else: ext_map2[k]=r[3]
    ...
    ne_pct=[round(ext_map2.get(m,0)/(nec_map2.get(m,0)+ext_map2.get(m,0))*100,1)
            if (nec_map2.get(m,0)+ext_map2.get(m,0))>0 else 0 for m in ne_months]
    nec_ext_data=json.dumps({'labels':ne_labels,'nec':ne_nec,'ext':ne_ext,'pct_ext':ne_pct})

    # ── Tab 5: Anomalie (anno selezionato) ──────────────────────────
    ...
    anomaly_threshold=None; anomalies=[]
    if len(spe_vals_a)>=3:
        avg_s=statistics.mean(spe_vals_a); std_s=statistics.stdev(spe_vals_a)
        anomaly_threshold=avg_s+std_s
        for ym in all_months_a:
            s=spe_map_a.get(ym,0)
            if s>anomaly_threshold:
                anomalies.append({'mese':ym,'spesa':s,'delta':s-avg_s,'delta_pct':(s-avg_s)/avg_s*100})
        anomalies.sort(key=lambda x:-x['spesa'])

    # ── Tab 6: Frequenza (anno selezionato) ─────────────────────────
    ...
    """, (freq_year,))
    freq_data=[{'cat':r[0],'cnt':r[1],'tot':round(r[2],2),'avg':round(r[3],2)} for r in freq_rows]

    # ── Tab 7: Sankey (anno selezionato) ────────────────────────────
    sank_year = request.args.get('sank_year', str(today.year))
    total_inc_sank = q(conn, "SELECT COALESCE(SUM(euro),0) FROM incomes WHERE user_id=1 AND strftime('%Y',date)=?", (sank_year,))[0][0]
    ...
    # Categorie (ordine fisso: extra rev + essential rev)
    ...
    def build_bar_chart(rows, cat_list, tmap):
        cat_m = defaultdict(float); det_m = defaultdict(list)
        for cat, euro, desc, date in rows:
            cat_m[cat] += euro
            det_m[cat].append((date, desc, euro))
        vals   = [cat_m.get(c, 0) for c in cat_list]
        colors = ['#ff7f0e' if tmap[c]=='extra' else '#1f77b4' for c in cat_list]
        def fmt(cat):
            items = det_m.get(cat, [])
            if not items: return '(nessuna spesa)'
    ...
    bil_tot_ent  = q(conn, "SELECT COALESCE(SUM(euro),0) FROM incomes WHERE user_id=1 AND strftime('%Y',date)=? AND strftime('%m',date)=?",
    ...
    ann_tot_ent  = q(conn, "SELECT COALESCE(SUM(euro),0) FROM incomes WHERE user_id=1 AND strftime('%Y',date)=?", (str(bil_anno),))[0][0]
    ...
    if stor_period == 'ytd':
        stor_start = datetime(today.year, 1, 1)
    elif stor_period == '6m':
        stor_start = today.replace(day=1)
        for _ in range(6): stor_start = (stor_start - timedelta(days=1)).replace(day=1)
    elif stor_period == '12m':
        m = today.month - 11; y = today.year
        if m <= 0: m += 12; y -= 1
        stor_start = datetime(y, m, 1)
    elif stor_period == '5y':
        stor_start = datetime(today.year - 4, 1, 1)
    else:
    ...
    stor_end = today.replace(day=last_day)
    ss, se = stor_start.strftime('%Y-%m-%d'), stor_end.strftime('%Y-%m-%d')

    stor_inc_rows = q(conn, "SELECT strftime('%Y',date) y, strftime('%m',date) m, SUM(euro) FROM incomes WHERE user_id=1 AND date BETWEEN ? AND ? GROUP BY y,m ORDER BY y,m", (ss, se))
    ...
    stor_months = []
    cur_m = stor_start.replace(day=1)
    while cur_m <= stor_end.replace(day=1):
        stor_months.append(f"{cur_m.year}-{cur_m.month:02d}")
        cur_m = (cur_m.replace(month=cur_m.month+1) if cur_m.month < 12 else cur_m.replace(year=cur_m.year+1, month=1))

    inc_map_s  = {f"{r[0]}-{r[1]}": r[2] for r in stor_inc_rows}
    ess_map_s  = {}; ext_map_s = {}
    for r in stor_exp_rows:
        k = f"{r[0]}-{r[1]}"
        if r[2]=='essential': ess_map_s[k] = ess_map_s.get(k,0)+r[3]
        else:                 ext_map_s[k] = ext_map_s.get(k,0)+r[3]
    ...
    h_sav  = [h_inc[i]-h_ess[i]-h_ext[i] for i in range(len(stor_months))]

    stor_tot_inc = sum(h_inc); stor_tot_ess = sum(h_ess); stor_tot_ext = sum(h_ext)
    stor_tot_exp = stor_tot_ess + stor_tot_ext; stor_tot_sav = sum(h_sav)
    stor_nm = len(stor_months) or 1
    stor_avg_inc = stor_tot_inc/stor_nm; stor_avg_exp = stor_tot_exp/stor_nm; stor_avg_sav = stor_tot_sav/stor_nm

    chart_hist_bil = json.dumps({'labels':stor_months,'essential':h_ess,'extra':h_ext,'savings':h_sav,'income':h_inc})

    def pct_s(val, base): return val/base*100 if base else 0
    def rule_status(pe, px, ps):
        if pe<=50 and px<=30 and ps>=20: return 'green'
        if pe<=60 and px<=40 and ps>=10: return 'yellow'
        return 'red'

    hist_rows_bil = []
    for i, m in enumerate(stor_months):
        inc=h_inc[i]; ess=h_ess[i]; ext=h_ext[i]; exp=ess+ext; sav=h_sav[i]
        pe=pct_s(ess,inc); px=pct_s(ext,inc); ps=pct_s(sav,inc)
        hist_rows_bil.append({'label':m,'inc':inc,'exp':exp,'ess':ess,'ext':ext,'sav':sav,
            'pct_exp':pct_s(exp,inc),'pct_ess':pe,'pct_ext':px,'pct_sav':ps,
            'status': rule_status(pe,px,ps) if inc>0 else None})
    tp_e=pct_s(stor_tot_ess,stor_tot_inc); tp_x=pct_s(stor_tot_ext,stor_tot_inc); tp_s=pct_s(stor_tot_sav,stor_tot_inc)
    hist_rows_bil.append({'label':'Totale','inc':stor_tot_inc,'exp':stor_tot_exp,'ess':stor_tot_ess,
        'ext':stor_tot_ext,'sav':stor_tot_sav,'pct_exp':pct_s(stor_tot_exp,stor_tot_inc),
        'pct_ess':tp_e,'pct_ext':tp_x,'pct_sav':tp_s,
        'status': rule_status(tp_e,tp_x,tp_s) if stor_tot_inc>0 else None,'is_total':True})
    ...
    return render_template('statistiche.html', empty=False,
        ...
        chart_bar_bil=chart_bar_bil,
        ann_tot_ent=ann_tot_ent, ann_tot_spe=ann_tot_spe, ann_ess=ann_ess, ann_ext=ann_ext, ann_risp=ann_risp,
        chart_bar_ann_bil=chart_bar_ann_bil,
        # tab storico
        stor_period=stor_period, stor_nm=stor_nm,
        stor_tot_inc=stor_tot_inc, stor_tot_exp=stor_tot_exp, stor_tot_sav=stor_tot_sav,
        stor_avg_inc=stor_avg_inc, stor_avg_exp=stor_avg_exp, stor_avg_sav=stor_avg_sav,
        stor_tot_ess=stor_tot_ess, stor_tot_ext=stor_tot_ext,
