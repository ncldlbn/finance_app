...
def calc_derived(row):
    """Calcola i campi derivati da un dict di una riga."""
    ...
    return dict(
        liquidita=liquidita, breve=breve, lungo=lungo,
        previdenza=previdenza, totale=totale
    )
...
def index():
    ...
    for i, r in enumerate(rows):
        if i < len(rows) - 1:
            r['variazione'] = r['totale'] - rows[i+1]['totale']
        else:
            r['variazione'] = None

    # Pre-fill edit form: default to current month or last entry
...
def save():
    ...
    conn = get_finance_conn()
    existing = conn.execute("SELECT id FROM patrimonio WHERE anno=? AND mese=?", (anno, mese)).fetchone()
    if existing:
        conn.execute(
